...
export function getUrgency(dueDate, status) {
  ...
  if (hoursLeft <= 24) return { label: `${formatDistanceToNowStrict(date)} left`, tone: 'urgent' };
  if (hoursLeft <= 72) return { label: `${formatDistanceToNowStrict(date)} left`, tone: 'warn' };
