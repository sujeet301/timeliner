...
function computeStreak(completedDates) {
  let streak = 0;
  let cursor = new Date(new Date().setHours(0, 0, 0, 0));
...
export default function AnalyticsPage() {
  ...
  useEffect(() => { dispatch(fetchTasks({ page: 1, limit: 500, sortBy: 'createdAt', order: 'desc' })); }, [dispatch]);
  ...
  const stats = useMemo(() => {
    ...
    const weekly = last7.map((day) => ({ day: format(day, 'EEE'), completed: completedDates.filter((d) => isSameDay(d, day)).length }));
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <StatCard icon={Flame} label="Current streak" value={stats.streak} sub={stats.streak === 1 ? 'day' : 'days'} tone="flame" />
          </div>
            ...
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stats.weekly}>
                  ...
                  <XAxis dataKey="day" tick={{ fill: 'var(--color-ink-muted)', fontSize: 12 }} axisLine={{ stroke: 'var(--color-border)' }} tickLine={false} />
                  <YAxis allowDecimals={false} tick={{ fill: 'var(--color-ink-muted)', fontSize: 12 }} axisLine={false} tickLine={false} width={28} />
                  <Tooltip contentStyle={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 8, fontSize: 12 }} cursor={{ fill: 'var(--color-surface-alt)' }} />
