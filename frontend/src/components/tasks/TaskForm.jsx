...
export default function TaskForm({ initialTask, onSubmit, onCancel, submitting }) {
  ...
  const [subtasks, setSubtasks] = useState(initialTask?.subtasks?.map((s) => ({ ...s })) || []);
  ...
  const toggleSubtask = (idx) => setSubtasks((prev) => prev.map((s, i) => (i === idx ? { ...s, done: !s.done } : s)));
  ...
  return (
    <form onSubmit={handleSubmit(submit)} className="flex flex-col gap-4">
      ...
      <Textarea label="Description" id="description" placeholder="Add any useful detail…" error={errors.description?.message} {...register('description')} />
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-1.5">
          {subtasks.map((s, idx) => (
            <div key={s._id || idx} className="flex items-center gap-2 rounded-lg border border-border bg-surface-alt px-2.5 py-1.5">
              <GripVertical size={14} className="shrink-0 text-ink-muted" />
              <input type="checkbox" checked={s.done} onChange={() => toggleSubtask(idx)} className="h-4 w-4 shrink-0 accent-[var(--color-primary)]" />
              <span className={`flex-1 truncate text-sm ${s.done ? 'text-ink-muted line-through' : 'text-ink'}`}>{s.title}</span>
              <button type="button" onClick={() => removeSubtask(idx)} aria-label="Remove subtask" className="shrink-0 rounded p-0.5 text-ink-muted hover:text-urgent">
                <X size={14} />
              </button>
            </div>
          ))}
