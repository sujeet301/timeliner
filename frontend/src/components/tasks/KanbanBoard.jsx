...
export default function KanbanBoard({ tasks, onOpen, onEdit, onDelete, onToggleComplete, onStatusChange }) {
  ...
  const columns = STATUS_OPTIONS.map((s) => ({ ...s, tasks: tasks.filter((t) => t.status === s.value) }));
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
      {columns.map((col) => (
        <div
          ...
          className={`flex flex-col gap-3 rounded-card border p-3 transition-colors ${dragOverStatus === col.value ? 'border-primary bg-primary-soft/40' : 'border-border bg-surface-alt/50'}`}
        >
          ...
          <div className="flex min-h-[6rem] flex-col gap-2.5">
            {col.tasks.length === 0 ? (
              ...
              col.tasks.map((task) => (
                <TaskCard key={task._id} task={task} draggable onDragStart={(e) => handleDragStart(e, task)} onOpen={onOpen} onEdit={onEdit} onDelete={onDelete} onToggleComplete={onToggleComplete} />
              ))
            )}
          </div>
        </div>
