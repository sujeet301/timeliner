...
export const Input = forwardRef(({ label, error, hint, id, className, ...props }, ref) => (
  <Field label={label} htmlFor={id} error={error} hint={hint}>
    <input id={id} ref={ref} className={clsx(fieldBase, error && 'border-urgent focus:border-urgent focus:ring-urgent', className)} {...props} />
  </Field>
));
...
export const Textarea = forwardRef(({ label, error, hint, id, className, ...props }, ref) => (
  <Field label={label} htmlFor={id} error={error} hint={hint}>
    <textarea id={id} ref={ref} rows={4} className={clsx(fieldBase, 'resize-y', error && 'border-urgent focus:border-urgent focus:ring-urgent', className)} {...props} />
  </Field>
));
...
export const Select = forwardRef(({ label, error, hint, id, className, children, ...props }, ref) => (
  <Field label={label} htmlFor={id} error={error} hint={hint}>
    <select id={id} ref={ref} className={clsx(fieldBase, 'cursor-pointer', error && 'border-urgent focus:border-urgent focus:ring-urgent', className)} {...props}>
      {children}
    </select>
  </Field>
));
...
export function Checkbox({ label, className, ...props }) {
  return (
    <label className={clsx('inline-flex items-center gap-2 text-sm text-ink cursor-pointer select-none', className)}>
      <input type="checkbox" className="h-4 w-4 rounded border-border text-primary focus:ring-primary accent-[var(--color-primary)]" {...props} />
      {label}
    </label>
  );
}
...
export function Toggle({ checked, onChange, label }) {
  return (
    <label className="inline-flex items-center gap-3 cursor-pointer select-none">
      <button type="button" role="switch" aria-checked={checked} onClick={() => onChange(!checked)} className={clsx('relative h-6 w-11 rounded-full transition-colors', checked ? 'bg-primary' : 'bg-border')}>
