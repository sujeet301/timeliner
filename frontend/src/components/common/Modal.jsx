  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-0 sm:p-4">
      ...
      <div
        ...
        aria-label={title}
        tabIndex={-1}
        className={`relative w-full ${widths[size]} sm:rounded-card bg-surface shadow-popover animate-slide-up max-h-full sm:max-h-[90vh] overflow-y-auto thin-scroll outline-none`}
      >
        <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-border bg-surface px-4 py-3 sm:px-5 sm:py-4">
          ...
          <button onClick={onClose} aria-label="Close" className="shrink-0 rounded-md p-1 text-ink-muted hover:bg-surface-alt hover:text-ink">
            <X size={18} />
          </button>
        </div>
