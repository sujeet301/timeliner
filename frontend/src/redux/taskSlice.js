...
export const fetchTasks = createAsyncThunk('tasks/fetchAll', async (filters, { rejectWithValue }) => {
  try {
    // Strip empty-string filters so they don't get sent as literal query params.
...
export const updateSubtask = createAsyncThunk(
  'tasks/updateSubtask',
  async ({ taskId, subtaskId, payload }, { rejectWithValue }) => {
    try {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(restoreTask.fulfilled, (state, action) => {
        state.trash = state.trash.filter((t) => t._id !== action.payload._id);
      })
      .addCase(permanentlyDeleteTask.fulfilled, (state, action) => {
        state.trash = state.trash.filter((t) => t._id !== action.payload);
      });
  },
});
