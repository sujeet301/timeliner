...
const getTasks = asyncHandler(async (req, res) => {
  const { search, status, priority, category, tag, sortBy = 'createdAt', order = 'desc', page = 1, limit = 20 } = req.query;
  ...
  const [tasks, total] = await Promise.all([
    Task.find(query).sort({ [sortField]: sortDir }).skip((pageNum - 1) * limitNum).limit(limitNum),
    Task.countDocuments(query),
  ]);

  res.json({ success: true, data: tasks, pagination: { page: pageNum, limit: limitNum, total, totalPages: Math.ceil(total / limitNum) } });
});
...
const createTask = asyncHandler(async (req, res) => {
  ...
  const task = await Task.create({ user: req.user._id, title, description, category, tags, priority, dueDate, subtasks });
...
const updateTask = asyncHandler(async (req, res) => {
  ...
  const editableFields = ['title', 'description', 'category', 'tags', 'priority', 'dueDate', 'subtasks'];
...
const deleteTask = asyncHandler(async (req, res) => {
  ...
  task.softDelete();
  await task.save();
  await Reminder.updateMany({ task: task._id, status: 'pending' }, { $set: { status: 'cancelled' } });
...
module.exports = { getTasks, getTrash, getTaskById, createTask, updateTask, updateTaskStatus, updateSubtask, deleteTask, restoreTask, permanentlyDeleteTask };
